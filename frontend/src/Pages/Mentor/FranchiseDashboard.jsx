...
const FranchiseDashboard = () => {
  ...
  const GetMentorAlldataAPI = async () => {
    try {
      const url = `${BASE_URL}/admin/allmentordata/${userId1}`;
      const headers = {
        "Content-Type": "application/json",
        Accept: "application/json",
        Authorization: `Bearer ${token}`
      ...
      const response = await axios.get(url, { headers });
      // console.log("Response of mentor data", response.data);

      // Extract counts from the response
      const mentorData = response.data.mentor;
      const totalEducators = mentorData.educators.length;
      const totalBatches = mentorData.batches.length;
      const totalStudents = mentorData.students.length;
      
      setEducators(mentorData.educators.length);
      setAllStudents(mentorData.students.length);
      setBatchData(mentorData.batches.length);
      setStudents(mentorData.students); // Set the students data for the table
  ...
  const cardData = [
    {
      title: "Assigned Educators",
      value: educators,
      percentage: "+23%",
      icon: <GiTeacher />,
      iconBg: "#51ab55",
      move: "/all-mentor-data",
      trend: "up",
    },
    {
  ...
  // Updated columns to show only enrollment number, first name, and last name
  const columns = [
  ...
  return (
    <>
      <PageTitle page={"Mentor Dashboard"} />
                  ...
                  components={{
                    Pagination: () => (
                      <Box sx={{ p: 1, display: "flex", justifyContent: "flex-end" }}>
                        <Typography variant="caption" color="text.secondary">
                          Showing {Math.min(rows.length, 5)} of {rows.length} students
                        </Typography>
                      </Box>
                    )
                  }}
