...
import { CalendarToday, Class, Schedule, VideoCall } from '@mui/icons-material';
...
const StudentDashboard = () => {
  ...
  const getTodaysClasses = async () => {
    try {
      ...
      // console.log("Today's classes:", response.data);
      setClassesData(response.data.data?.classes || []);
    } catch (error) {
      // console.error("Error fetching today's classes:", error);
      setError("You Don't have any class");
    }
  };
  ...
  const Myprofile = async () => {
    try {
      const url = `${BASE_URL}/admin/profile/${studentId}/${user}`;
      const headers = {
        "Content-Type": "application/json",
        Accept: "application/json",
        Authorization: `Bearer ${token}`
          ...
          <Grid item xs={12} md={3}>
            ...
            >
              ...
              <Typography variant="h6" fontWeight="bold">
                {adminProfile ? `${adminProfile.firstname} ${adminProfile.lastname}` : 'Student Dashboard'}
              </Typography>
              <Typography variant="body2" sx={{ opacity: 0.8, mt: 1 }}>
                {adminProfile ? `Welcome back, ${adminProfile.firstname}` : 'Welcome back'}
              </Typography>
              <Typography variant="caption" sx={{ opacity: 0.8 }}>
                {adminProfile?.email}
              </Typography>
              <Chip
                label={adminProfile?.role || 'Student'}
                size="small"
                sx={{ 
                  mt: 2, 
                  color: "white", 
          ...
          <Grid item xs={12} md={8}>
            ...
            >
              <Box sx={{ 
                ...
                <Class style={{ marginRight: 10, fontSize: 24 }} />
                <Typography variant="h6" fontWeight="bold">
              ...
              {classesData.length > 0 ? (
                    ...
                    <Grid item xs={12} key={index}>
                      ...
                      >
                        ...
                        <Box display="flex" alignItems="center" mb={1}>
                          <Schedule color="action" sx={{ mr: 1, fontSize: 20 }} />
                          <Typography>
                            {format(parseISO(classItem.classDate), 'h:mm a')}
                          </Typography>
                        </Box>
                        
                        <Box display="flex" alignItems="center" flexWrap="wrap" gap={1} mt={2}>
                          {classItem.schedule.days.map((day, i) => (
                            <Chip 
                              key={i}
                              label={day} 
                              size="small"
                ...
                >
                  <CalendarToday sx={{ fontSize: 60, color: "#9e9e9e", mb: 2 }} />
